...
/** Outward direction vector of a port based on its relX/relY position. */
function getPortDir(relX: number, relY: number): { dx: number; dy: number } {
  if (relY <= 0.1) return { dx: 0, dy: -1 };  // top
  if (relY >= 0.9) return { dx: 0, dy:  1 };  // bottom
  if (relX <= 0.1) return { dx: -1, dy: 0 };  // left
  return                 { dx:  1, dy: 0 };   // right
}

/**
 * Build the SVG path string for a belt segment given its routing mode.
 *  default – vertical cubic Bezier (original look)
 *  straight – orthogonal L/Z-route through midpoint
 *  curve    – direction-aware cubic Bezier using port tangents
 */
function buildBeltPath(
  mode: BeltMode,
  p1: { x: number; y: number },
  p2: { x: number; y: number },
  fromRelX: number, fromRelY: number,
  ...
  if (mode === 'straight') {
    const dir = getPortDir(fromRelX, fromRelY);
    if (dir.dy !== 0) {
      const midY = (y1 + y2) / 2;
  ...
  if (mode === 'curve') {
    const fd = getPortDir(fromRelX, fromRelY);
    const td = getPortDir(toRelX,   toRelY);
    const dx = x2 - x1, dy = y2 - y1;
    const handle = Math.max(Math.sqrt(dx * dx + dy * dy) * 0.45, 50);
    return `M ${x1} ${y1} C ${x1 + fd.dx * handle} ${y1 + fd.dy * handle}, ` +
           `${x2 + td.dx * handle} ${y2 + td.dy * handle}, ${x2} ${y2}`;
  }

  // default: vertical-biased cubic Bezier (original behaviour)
  const dy = y2 - y1;
  return `M ${x1} ${y1} C ${x1} ${y1 + dy * 0.5}, ${x2} ${y2 - dy * 0.5}, ${x2} ${y2}`;
}

/** Arrow glyph pointing in a port's outward direction. */
function portArrow(relX: number, relY: number): string {
  const { dx, dy } = getPortDir(relX, relY);
  if (dy < 0) return '▲';
  if (dy > 0) return '▼';
  if (dx < 0) return '◀';
  return '▶';
...
export function SandboxCanvas() {
  ...
  // Belt routing mode — cycles with R when belt tool is active
  const [beltMode, setBeltMode] = useState<BeltMode>('default');
  ...
  // R cycles belt routing mode while the belt tool is active
  useEffect(() => {
    const MODES: BeltMode[] = ['default', 'straight', 'curve'];
    const handler = (e: KeyboardEvent) => {
      if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement) return;
      if (e.ctrlKey || e.metaKey || e.altKey) return;
      if (e.key.toLowerCase() === 'r' && toolMode === 'belt') {
        setBeltMode((prev) => MODES[(MODES.indexOf(prev) + 1) % MODES.length]);
      }
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
    ...
    return ports.map((port) => {
      ...
      return (
        ...
        >
          ...
          {/* Port direction arrow — points in the port's outward direction */}
          {toolMode === 'belt' && (
            <text
              x={px} y={py}
              textAnchor="middle" dominantBaseline="middle"
  ...
  const renderBelt = (belt: SandboxBelt) => {
    ...
    const path = buildBeltPath(
      belt.mode ?? 'default',
      fp1, fp2,
      fromPort.relX, fromPort.relY,
      toPort.relX,   toPort.relY,
    );
  ...
  const renderBeltPreview = () => {
    if (!beltDraw) return null;
    const fromMachine = placedMachines.find((m) => m.instanceId === beltDraw.fromMachineId);
    if (!fromMachine) return null;
    const fromPorts = getMachinePorts(fromMachine.machineId);
    const fromPort  = fromPorts.find((p) => p.id === beltDraw.fromPortId);
    ...
    const path = buildBeltPath(beltMode, fp1, mousePos, fromPort.relX, fromPort.relY, fromPort.relX, fromPort.relY);
    return (
      <path
        d={path}
        stroke="#f48721"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <g className="sandbox-ghost">{renderGhost()}{renderMachineDragGhost()}{renderBeltPreview()}</g>
      ...
      {/* Belt draw mode HUD — also shown (without the "complete" prompt) just to show mode picker */}
      {toolMode === 'belt' && (
        <div className="sandbox-belt-hud">
          {beltDraw
            ? <span>🔗 Click an <strong>input port</strong> to complete belt</span>
            : <span>🔗 Click an <strong>output port</strong> to start belt</span>
          }

          {/* Routing mode picker */}
          <div className="belt-mode-picker">
            {(['default', 'straight', 'curve'] as BeltMode[]).map((m) => (
              <button
                key={m}
                className={`belt-mode-btn${beltMode === m ? ' is-active' : ''}`}
                onClick={() => setBeltMode(m)}
                title={`${m} routing (R to cycle)`}
              >
                {m === 'default' ? '〜' : m === 'straight' ? '⌐' : '⌒'}
                <span>{m[0].toUpperCase() + m.slice(1)}</span>
              </button>
            ))}
          </div>

          <span className="hud-esc-hint">R to cycle mode · ESC to cancel</span>
          {beltDraw && <button onClick={() => setBeltDraw(null)}>Cancel</button>}
        </div>
      )}
